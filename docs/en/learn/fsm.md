...
1. The bot asks for a name → the user enters a name
2. The bot asks for an age → the user enters an age
3. The bot asks for a city → the user enters a city
4. The bot displays a summary
...
### Step 2. Creating storage and a dispatcher
...
!!! info
    `MemoryStorage` stores states in RAM. After the bot restarts, all states are reset. 
    This is enough for development and testing. For production, you can implement [your own persistent storage](#_16).

### Step 3. Declaring states
...
You do not define it manually — it is generated from the class name and the attribute name.
...
!!! Tip
    Note that `state: FSMContext` is passed to the handler automatically as a parameter. 
    You do not need to create it manually — the library injects the required object for you.
...
Groups can be nested inside one another. This is useful for complex multi-step forms where one scenario contains separate logical blocks. 
For example, during registration, you can move the address into a separate nested group so that the `city`, `street`, and `zip_code` states are associated specifically with the address and are not mixed with the main form steps.
...
=== "Clear state"
    ...
    !!! warning "Difference between clear and set_state(None)"
...
This handler will run only if the state has not been set yet or has already been cleared:
...
!!! warning "Handler order matters!"
    ...
    If you have a handler with `Command("cancel")` and a handler with `Form.name`, **register commands first**, 
    otherwise `/cancel` will be handled by the state handler and will not work as cancellation:
