---
title: Фильтры
description: Как направить события к нужным обработчикам с помощью фильтров
...
Вы можете создавать собственные фильтры для тонкой настройки логики обработки событий. 
Это позволяет вынести проверку условий (например, наличие пользователя в базе данных или проверку прав доступа) в отдельный переиспользуемый компонент.
...
- **Метод `__call__`:** Внутри класса обязательно должен быть реализован метод __call__. Без него объект класса не будет считаться «вызываемым», и фильтр не сработает.
- **Возвращаемое значение:** Метод __call__ должен возвращать True (если событие подходит под условия) или False (если событие должно быть проигнорировано данным хендлером).
...
После того как класс фильтра создан, его можно использовать в декораторах хендлеров точно так же, как стандартные фильтры **Command** или **F**. 
При регистрации хендлера просто передайте экземпляр вашего класса в качестве аргумента.
...
В **python-trueconf-bot** вы можете комбинировать несколько фильтров в одном хендлере. 
По умолчанию, если вы перечисляете фильтры через запятую, они работают по логике **AND** (И) — хендлер сработает только в том случае, если **все** фильтры вернут `True`.
...
### Связка Command и аргументов команды с MagicFilter
